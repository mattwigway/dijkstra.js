...
Graph=function(graph){var instance=this;_.bindAll(this,"classForNode","setSelectedNode","step","run","fillForNode","showPath","hidePath");this.graph=graph;var width=800;var height=500;this.costscale=d3.scale.linear().range([0,1]);// used to display widths
this.widthscale=d3.scale.log().range([.3,10]);this.indexGraph();// perform initial rendering
this.svg=d3.select("#graph").append("svg").attr("width",width).attr("height",height);this.xscale=d3.scale.linear().domain([this.min_x,this.max_x]).range([3,width-3]);this.yscale=d3.scale.linear().domain([this.max_y,this.min_y]).range([3,height-3]);// draw the edges
...
this.svg.on("mouseout",function(){instance.hidePath()});nd.append("title").text(function(d){return d.name});this.render();d3.select("#run").on("click",this.run);d3.select("#step").on("click",this.step);this.done=false};/**
